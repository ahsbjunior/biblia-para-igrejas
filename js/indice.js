...
function filtraLivrosPorTexto(texto) {
  ...
  for (var i = 0; i < tds.length; i++) {
    ...
    for (var j = 0; j < ps.length; j++) {
      if (!removeDiacritics(ps[j].innerText.toLowerCase()).match(textoRegex.toLowerCase())) tds[i].style.opacity = 0.15;
...
function adicionaRegex(texto) {
  ...
  var textoArray = removeDiacritics(texto.toLowerCase()).split("")
  var arrayRetorno = [".*"]
...
// função obtida em
// https://stackoverflow.com/questions/18123501/replacing-accented-characters-with-plain-ascii-ones/18123985
